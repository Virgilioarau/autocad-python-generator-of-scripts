...
def carregar_arquivo():
    ...
    if file_path:
        try:
            ...
            info_label.config(
                text="Arquivo não encontrado. Verifique o caminho e o nome do arquivo.")
        except Exception as e:
            info_label.config(text=f"Erro ao carregar arquivo: {e}")


def verifica_codigos(row):
    colunas = ['FO 0180', 'FO 90270', 'FO 0',
               'FO 90', 'FO 180', 'FO 270', 'FO AD']
    ...
    if 'Leste' in df.columns and 'Norte' in df.columns:
        try:
            ...
            for index, row in df.iterrows():
                ...
                colunas = ['FO 0180', 'FO 90270', 'FO 0',
                           'FO 90', 'FO 180', 'FO 270', 'FO AD']
                # Flag para identificar se foi encontrado algum valor não vazio
                ...
                for coluna in colunas:
                    valor = str(row[coluna])
                    if valor.strip():
                        # Adiciona os valores identificados na coluna
                        valores_por_coluna[coluna] = valor.split()
                        valor_encontrado = True  # Marca que um valor foi encontrado

                # Se valores forem encontrados, considerar a coluna com mais códigos
                if valor_encontrado:
                    coluna_mais_codigos = max(
                        valores_por_coluna, key=lambda k: len(valores_por_coluna[k]))
                    valores = valores_por_coluna[coluna_mais_codigos]

                # Formar o script de coordenadas
                if valores:
                    posicoes = [f"{i}ª_POSIÇÃO_{codigo}" for i,
                                codigo in enumerate(valores, start=1)]
            ...
            if file_to_save:
                ...
                file_to_save.close()
                info_label.config(
                    text="Scripts para COORDENADAS gerados e salvos com sucesso!")
        except Exception as e:
            info_label.config(
                text=f"Erro ao gerar/salvar scripts para COORDENADAS: {e}")
    else:
        info_label.config(
            text="Colunas 'Leste' e 'Norte' necessárias para gerar COORDENADAS não encontradas.")
    ...
    if 'Leste' in df.columns and 'Norte' in df.columns and 'POSTE' in df.columns and 'POSIÇÃO' in df.columns:
        try:
            ...
            if file_to_save:
                ...
                file_to_save.close()
                info_label.config(
                    text="Scripts para TEXTO DO POSTE gerados e salvos com sucesso!")
        except Exception as e:
            info_label.config(
                text=f"Erro ao gerar/salvar scripts para TESTO DO POSTE: {e}")
    else:
        info_label.config(
...
generate_mtext_button = tk.Button(
    root, text="TEXTO DO POSTE", command=gerar_scripts_mtext)
